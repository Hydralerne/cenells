...
$querylike = mysql_query("SELECT * FROM likecom WHERE post_id='$postid' AND for_id='$commid' AND type='$type' AND do='dislike' ORDER BY id DESC LIMIT $limits OFFSET $offset");
...
<?php
$querycounc = mysql_query("SELECT id FROM likecom WHERE post_id='$postid' AND for_id='$commid' AND type='$type' AND do='dislike' ORDER BY id LIMIT 1");
$numrowscom = mysql_result($querycounc,0);
if($numrowscom == $empty){
echo '
<script>
$(".addlimit_whocomdislike").fadeOut(100);
</script>
';
}else {
echo '
<script>
$(".addlimit_whocomdislike").fadeIn(100);
</script>
';
}
}
