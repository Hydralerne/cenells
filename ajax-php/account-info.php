...
$use = addslashes(htmlspecialchars(htmlentities(strip_tags(trim($_GET['userid'])))));
...
$selectview = mysql_query("SELECT COUNT(id) FROM view WHERE for_id='$use' AND type='profile'",$dbh1);
$accviewcount = mysql_result($selectview,0);

$selectpost = mysql_query("SELECT COUNT(id) FROM view WHERE to_id='$use' AND type='post'",$dbh1);
...
<span><p>عدد المشاهدات للتدوينات : </p> <a><?php if(empty($countviewp)){echo 'لايوجد معلومات لعرضها';}else{echo $countviewp;} ?></a></span>
</div>
</div>
</div>
...
<span><p>اجمالي عدد المشاركات :</p> <a>لايوجد معلومات لعرضها</a></span>
</div>
</div>
</div>
